...
// Esta clase contiene operaciones comunes a un socket cliente/servidor
public class UDPClient{

  // Constantes
  final int MAX_SIZE_QUEUE = 100;
  final int SENDED_PACKAGES_PER_CLIENT = 200;
  ...
  //ArrayBlockingQueue<DatagramPacket> lbqueue = new ArrayBlockingQueue<DatagramPacket>(MAX_SIZE_QUEUE);
...
//************************************************************************************************************   
  ...
  private void verificaMensaje(){
    ...
    // La queue debe enviar paquetes mientras tanto ....
    // Despiertan todos y vuelven a enviar todos paquetes

    //System.out.println("Multicast info recibida: " + dgp.getData());
...
//************************************************************************************************************   
  ...
  public Map recibirPaquetesQueue(){
    ...
    // de cada uno de los paquetes que enviaran los threads
        System.out.println("Iteracion limite: " + UserAPPS.MAX_USER_APPS_PACKETS * numClients);
    ...
    //while(cont <= UserAPPS.MAX_USER_APPS_PACKETS * numClients){ // 100 * 3
    while (System.currentTimeMillis() < end){
       cont++;

       end = start + 500*1000; // 60 seconds * 1000 ms/sec
       
       //System.out.println("Valor contador: " + cont);

       if(cont % 50 == 0){
        System.out.println("Procesando paquete... " + cont);
       }
       
       //System.out.println("El tamaño de la LinkedBlockingQueue es: " + abqueue.size()); // Para depurar
       ...
       // INTERRUPCIONES PARA ATENDER LOS PAQUETES EN LA COLA
       // Simulacion de interrupciones       
       
       // EL POLLING SE ACTIVA AL 75% DE LA CAPACIDAD DE LA QUEUE Y SI ADEMAS ESTA ACTIVADA LA OPCION
       //if(abqueue.size() >= (MAX_SIZE_QUEUE*75)/100 && polling != 0){
       if(polling != 0 && abqueue.size() >= 5){
        //System.out.println("Realizando polling ...");
        //int contador = 0;
       ...
       //if(lbqueue.size() % SENDED_PACKAGES_PER_CLIENT == 0 && lbqueue.size()>0){
       //if(abqueue.size() % SENDED_PACKAGES_PER_CLIENT == 0 && abqueue.size()>0){
       if(abqueue.size() >= 5){        
        //System.out.println("Tamanio de la queue: " + abqueue.size());                  
        // Para enviar 50 paquetes desde la queue hacia el dispositivo E/S correspondiente
        //enviarPaqueteQueue(SENDED_PACKAGES_PER_CLIENT);
        enviarPaqueteQueue(5);
        abqueue.clear();
       }
    }
